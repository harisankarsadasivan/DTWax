...
__global__ void DTW(reference_coefficients *ref, val_t *query, val_t *dist,
  ...
  val_t last_col_penalty_wr_reg;             // used to store last col of matrix
  val_t penalty_last_col_rd_reg;
...
#pragma unroll(32)
#endif
  for (idxt wave = 1; wave <= SMEM_BUFFER_PLUS_WARP_SIZE_MINUS_ONE;
       wave++) { // waves loop
    ...
    last_col_penalty_wr_reg = __shfl_down_sync(ALL, last_col_penalty_wr_reg, 1);
    if (threadIdx.x == WARP_SIZE_MINUS_ONE)
      last_col_penalty_wr_reg = penalty_here[RESULT_REG];
...
#pragma unroll(32)
#endif
  for (idxt wave = SMEM_BUFFER_PLUS_WARP_SIZE; wave <= NUM_WAVES;
       wave++) { // waves loop
    ...
    if ((wave & WARP_SIZE_MINUS_ONE) == WARP_SIZE_MINUS_ONE) {
   ...
   * ---------------------------------- */
#if REF_BATCH > 1
  for (idxt ref_batch = 1; ref_batch < REF_BATCH - 1;
       ref_batch++) { // ref_batch loop
#ifdef NV_DEBUG
    printf("refbatch=%0d\n", ref_batch);
#endif
...
#pragma unroll(32)
    ...
    for (idxt wave = 1; wave <= SMEM_BUFFER_PLUS_WARP_SIZE_MINUS_ONE;
      ...
      last_col_penalty_wr_reg =
          __shfl_down_sync(ALL, last_col_penalty_wr_reg, 1);
      if (threadIdx.x == WARP_SIZE_MINUS_ONE)
        last_col_penalty_wr_reg = penalty_here[RESULT_REG];
      ...
      if (threadIdx.x == 0) {
        if (wave <= SMEM_BUFFER_SIZE_MINUS_ONE)
          ...
          penalty_left = penalty_last_col[(wave - SMEM_BUFFER_SIZE)];
      }

      // Find min of segment and then shuffle up for sDTW
      if (wave >= QUERY_LEN) {
        if (wave >= QUERY_LEN_PLUS_ONE)
          min_segment = __shfl_up_sync((ALL), min_segment, 1);
        for (idxt i = 0; i < SEGMENT_SIZE; i++) {
          min_segment = FIND_MIN(min_segment, penalty_here[i]);
        }
...
#pragma unroll(32)
    ...
    for (idxt wave = SMEM_BUFFER_PLUS_WARP_SIZE; wave <= NUM_WAVES;
      ...
      if ((wave & (WARP_SIZE_MINUS_ONE)) == 0) {
        new_query_val = query[blockIdx.x * QUERY_LEN + wave + threadIdx.x];
        penalty_last_col_rd_reg = penalty_last_col[wave + threadIdx.x];
      }
      ...
      if (threadIdx.x == 0) {
        ...
        penalty_left = penalty_last_col_rd_reg;
      }
      penalty_last_col_rd_reg =
          __shfl_down_sync(ALL, penalty_last_col_rd_reg, 1);
      // Find min of segment and then shuffle up for sDTW
      if (wave >= QUERY_LEN) {
        if (wave >= QUERY_LEN_PLUS_ONE)
          min_segment = __shfl_up_sync((ALL), min_segment, 1);
...
#pragma unroll(32)
#endif
  for (idxt wave = 1; wave <= SMEM_BUFFER_PLUS_WARP_SIZE_MINUS_ONE; wave++) {
...
#pragma unroll(32)
  ...
  for (idxt wave = SMEM_BUFFER_PLUS_WARP_SIZE; wave <= NUM_WAVES; wave++) {
    ...
    penalty_last_col_rd_reg = __shfl_down_sync(ALL, penalty_last_col_rd_reg, 1);
  ...
  if (threadIdx.x == WARP_SIZE_MINUS_ONE) {
    ...
    // #ifdef NV_DEBUG
    printf("min_segment=%0f, tid=%0d, blockid=%0d, q=%0f, p=%0f\n", min_segment,
           threadIdx.x, blockIdx.x, query[blockIdx.x * QUERY_LEN],
